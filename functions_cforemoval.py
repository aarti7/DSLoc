...
def get_cfo_either_lin_or_db_pwr(fn, df_allrx, df_allti, gt_loc_df, fsr, lpf_fc, exp_start_timestampUTC, degreeforfitting, pwr_threshold):
    '''
    '''
    print("\n\nCalculating the cfo.... ")
        ...
        for p in range(0, n_endpoints_is_subplots): #    [1]:#     [3]:# 
            ...
            if this_speed.values[0] == 0: # to ensure only for cases  when there was no motion. 
                ...
                if 0:
                    ...
                    if len(listofidxsofresult_lin) ==0: continue #print("no peak detected!!!")
    ...
    print("high_SNR_n_list", high_SNR_n_list)
    # plot_all_off_dictionaries(ff, f"{args.dirdata}".split('meas_')[1], cfo_summary_dict, "./", f'{int(time.time())}', degreeforfitting)
    # pdb.set_trace()
    return summary_cfo_dict, no_measr_time_idx_n, no_gps_mesrnt_idx_n , cfo_mthd, high_SNR_n_list
